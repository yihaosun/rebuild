  ...
  <div>
   ...
   'color':' #fff',
    ...
    'text-align': 'left'
        ...
        >
          <el-table-column prop="OUTLAY_CODE" label="功能科目编码"></el-table-column>
          <el-table-column prop="OUTLAY_NAME" label="功能科目名称"></el-table-column>
...
<script>
import { lookJjkmList } from "@/api/gov/reportappear";
import { dropDownGnkmList } from "@/api/gov/reportappear";
import { saveJjkmList } from "@/api/gov/reportappear";
import { commitJjkmList } from "@/api/gov/reportappear";
import { backJjkmList } from "@/api/gov/reportappear";
...
export default {
  ...
  this.loading = false;
   this.dialogVisible=false
    ...
    //保存
    updateDataSource() {
      saveJjkmList(this.tableData).then(response => {
        if (response) {
          setTimeout(() => {
            this.loading = false;
          }, 1000);
