  ...
  <div>
   ...
   'color':' #fff',
    ...
    'text-align': 'left'
        ...
        >
          <el-table-column prop="GOV_OUTLAY_CODE" label="功能科目编码"></el-table-column>
          <el-table-column prop="GOV_OUTLAY_NAME" label="功能科目名称"></el-table-column>
...
<script>
import { lookJbzcList } from "@/api/gov/reportappear";
import { dropDownGnkmList } from "@/api/gov/reportappear";
import { saveJbzcList } from "@/api/gov/reportappear";
import { commitJbzcList } from "@/api/gov/reportappear";
import { backJbzcList } from "@/api/gov/reportappear";
...
export default {
  ...
  this.loading = false;
   this.dialogVisible=false
    ...
    //保存
    updateDataSource() {
      saveJbzcList(this.tableData).then(response => {
        if (response) {
          setTimeout(() => {
            this.loading = false;
          }, 1000);
